        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-2 bg-secondary/50 px-4 py-2 rounded-xl">
            ...
            <button 
              ...
              className="px-3 py-1.5 bg-primary/20 hover:bg-primary/30 text-primary text-xs font-bold rounded-lg transition-colors flex items-center gap-1"
            >
            ...
            <button 
              ...
              disabled={isGenerating}
              className="px-3 py-1.5 bg-accent/20 hover:bg-accent/30 text-accent text-xs font-bold rounded-lg transition-colors flex items-center gap-1"
            >
        ...
        {leads.length === 0 ? (
          ...
          leads.map(lead => (
            <motion.div 
              ...
              key={lead.id} 
              className={`p-4 rounded-xl bg-card border transition-all ${selectedIds.has(lead.id) ? 'border-primary shadow-[0_0_15px_rgba(147,51,234,0.15)]' : 'border-border hover:border-primary/50'}`}
            >
              <div className="flex items-start gap-4">
                <input 
                  ...
                  className="mt-1.5 w-4 h-4 rounded border-border text-primary focus:ring-primary bg-background"
                />
                
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    <h4 className="font-bold text-base truncate">{lead.company_name}</h4>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider ${getBadgeColor(lead.quality_score || 0)}`}>
                      Score {lead.quality_score || 0}
                    </span>
                    {getTopFactors(lead.score_factors).map(factor => (
                      <span key={factor} className="px-2 py-0.5 rounded-full bg-secondary text-muted-foreground text-[10px] capitalize">
                        {factor}
                      </span>
                    ))}
                  </div>
                  ...
                  {lead.ai_hook_draft && generationStatus[lead.id] !== 'generating' && (
                    <div className="p-3 bg-secondary/30 rounded-lg text-sm text-foreground/80 mb-3 border border-border/50">
                      {lead.ai_hook_draft}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a 
                      ...
                      target="_blank"
                      className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 text-xs font-medium transition-colors ${lead.phone_e164 ? 'bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20' : 'bg-secondary text-muted-foreground opacity-50 cursor-not-allowed'}`}
                    ...
                    <a 
                      ...
                      className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 text-xs font-medium transition-colors ${lead.email_verified ? 'bg-primary/10 text-primary hover:bg-primary/20' : 'bg-secondary text-muted-foreground opacity-50 cursor-not-allowed'}`}
                    ...
                    <a 
                      ...
                      className={`px-3 py-1.5 rounded-lg flex items-center gap-1.5 text-xs font-medium transition-colors ${lead.phone_e164 ? 'bg-blue-500/10 text-blue-500 hover:bg-blue-500/20' : 'bg-secondary text-muted-foreground opacity-50 cursor-not-allowed'}`}
                    >
                    ...
                    <select 
                      ...
                      className="px-2 py-1.5 text-xs bg-background border border-border rounded-lg text-muted-foreground focus:outline-none focus:border-primary"
                    >
